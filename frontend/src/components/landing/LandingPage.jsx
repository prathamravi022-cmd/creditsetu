...
export default function LandingPage() {
  ...
  function CtaLink(props) { if (isAuthenticated) { return <Link to="/get-started" {...props} />; } return <SignInButton mode="modal" afterSignInUrl="/get-started"><button {...props} /></SignInButton>; }
  ...
  return (<div className="min-h-screen">
    ...
    <nav className="fixed top-1 left-0 right-0 z-50 cs-nav shadow-sm">
      <div className="max-w-7xl mx-auto px-6 lg:px-16 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3"><LangToggle /><CtaLink className="hidden sm:inline-flex px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-[#FF9933] hover:bg-[#e68a2d] transition-colors shadow-md">{t("landing.hero.get_started_btn")}</CtaLink><button onClick={function(){setMenuOpen(!menuOpen)}} className="lg:hidden w-10 h-10 rounded-lg flex items-center justify-center text-gray-600 hover:bg-gray-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#138808]" aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} title={menuOpen ? 'Close menu' : 'Open menu'}>{menuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button></div>
      </div></nav>
      ...
      <aside role="dialog" aria-modal="true" aria-label="Navigation menu" className={"absolute top-0 right-0 h-full w-80 max-w-[85vw] bg-white dark:bg-gray-900 border-l border-gray-100 dark:border-gray-800 shadow-2xl flex flex-col transition-transform duration-300 ease-out " + (menuOpen ? "translate-x-0" : "translate-x-full")}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-5 space-y-1.5">
          ...
          <div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800">
            <CtaLink onClick={function(){setMenuOpen(false)}} className="flex items-center justify-center gap-2 px-5 py-3.5 min-h-[48px] rounded-xl text-sm font-semibold text-white bg-[#FF9933] hover:bg-[#e68a2d] transition-colors shadow-md">{t("landing.hero.get_started_btn")} <ArrowRight className="w-4 h-4" /></CtaLink>
          </div>
        </nav>
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        <div className={"max-w-2xl transition-all duration-700 ease-out " + (heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")} style={{transitionDelay:"0.1s"}}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <CtaLink className="inline-flex items-center justify-center gap-2 px-7 py-3.5 sm:px-8 sm:py-4 rounded-lg text-white font-semibold bg-[#138808] hover:bg-[#0f6d06] transition-colors shadow-lg text-sm sm:text-base">{t("landing.hero.cta")} <ArrowRight className="w-5 h-5" /></CtaLink>
