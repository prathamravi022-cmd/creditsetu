...
for state in state_districts:
    ...
    for d in state_districts[state]:
        if d not in seen:
            seen.add(d)
            cleaned.append(d)
    state_districts[state] = cleaned

total = sum(len(d) for d in state_districts.values())
print(f"Total: {len(state_districts)} states/UTs, {total} districts")

outpath = os.path.join(os.path.dirname(__file__), 'stateDistricts.js')
with open(outpath, 'w', encoding='utf-8') as f:
    f.write('/**\n')
    f.write(' * All 36 Indian States & Union Territories with real district names.\n')
    f.write(' * Source: Census of India / data.gov.in\n')
    f.write(' * Used by OnboardingWizard for state -> district cascading dropdown.\n')
    f.write(' */\n\n')
    f.write('export const STATE_DISTRICTS = {\n')
    for state in sorted(state_districts.keys()):
        districts = state_districts[state]
        items = ", ".join(f'"{d}"' for d in districts)
        f.write(f'  "{state}": [{items}],\n')
    f.write('};\n\n')
    f.write(f'export const INDIAN_STATES = Object.keys(STATE_DISTRICTS).sort();\n\n')
    f.write(f'export const TOTAL_DISTRICTS = {total};\n')
